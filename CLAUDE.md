...
**Drag-to-reorder is back** (2026-04-22), replacing the arrow buttons. A dedicated square grab handle sits left of the rank number on your own tab; pointerdown on the handle clones the row into a floating element that follows the finger, paints a thick black bar on the nearest gap as a drop indicator, and commits the whole new order via `PUT /api/list/:id/my-ranks` on release. The list does **not** re-render during a drag — only classes flip on entries to show the indicator. Auto-scroll near the viewport edges is a small rAF loop (browsers don't do it for custom pointer drags). See Section 8 of `app.js`. This design specifically fixes Bug 8's three failure modes (see `bugfix.txt`): (a) the whole row was the drag target → now only the handle is, (b) list re-rendered mid-drag losing pointer capture → now nothing re-renders, (c) broken drags committed partial state → now one commit at drop or nothing.
